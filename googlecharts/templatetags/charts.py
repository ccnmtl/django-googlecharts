...
_encoding_chars = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz" + \
                  "0123456789-."
...
def encode_extended(values, value_range):
    """Encode data using Google's "extended" encoding for the most
    granularity."""
...
chxp=0,%s\
    " alt="" />' % (
        ...
        len(data),
        data[-1],
        100 * (float(data[-1]) / (maxvalue-minvalue)))
...
class Chart(object):
    ...
    def __init__(self):
        # Use a OrderedDict for the options so they are added in a
    ...
    def url(self):
        ...
        if not self.datarange:
            maxvalue = max(
                max(d) for d in chain(
                    self.datasets, self.hidden_datasets) if d)
            minvalue = min(
                min(d) for d in chain(
                    self.datasets, self.hidden_datasets) if d)
        ...
        if "chds" in self.options or self.options.get('cht', None) == 'gom':
            ...
            data = "|".join(
                encode_text(d) for d in chain(
                    self.datasets, self.hidden_datasets))
            encoded_data = "t%d:%s" % (len(self.datasets), data)
        else:
            # extended encoding otherwise
            data = extended_separator.join(
                encode_extended(d, self.datarange) for d in chain(
                    self.datasets, self.hidden_datasets))
        ...
        url = "%s?%s&chd=%s" % (self.BASE, urlencode(self.options),
                                encoded_data)
...
def option(tagname, multi=None, nodeclass=ChartOptionNode):
    """
    Decorator-helper to register a chart-foo option tag. The decorated
    function will be called at resolution time with the proper arity
    (determined from inspecting the decorated function). This callback
    should return a dictionary which will be used as arguments in the
    chart URL.
    """
    def decorator(func):
...
def chart_auto_colors(color, item_label_list):
    '''Takes a starting color and a list of labels and creates the
    correct number of colors, storing the correspondance between the
    labels and colors for later use in the context.'''
    ...
    final_color_map = OrderedDict()

    # Map our final color values to the label that will be associated with them
    for index, c in enumerate(colors):
        final_color_map[c] = item_label_list[index]

    # Values which begin with an underscore won't be passed on to
    # Google but will end up in the request context.
...
def chart_marker(marker, color, dataset_index, data_point, size):
    ...
    return {
        "chm": smart_join(
            ",", marker, color, dataset_index, data_point, size)
    }
...
def data_point_labels(dataset_index, iterable):
    ...
    You will need to provide an iterable yielding (label_type,
    label_contents, color, data_point, size, priority)

    And keep in mind that flags and text labels require a very
    different `label_contents` than a set of numbers.
    """
    try:
        it = iter(iterable)
    except TypeError:
    ...
    for m in it:
        ...
        labels.append(
            smart_join(",", label + label_contents, color,
                       dataset_index, data_point, size, priority))
    return {"chm": smart_join("|", *flatten(labels))}
...
def axis(parser, token):
    ...
    if len(bits) == 2:
        ...
        if bits[2].lower() != "hide":
            raise template.TemplateSyntaxError(
                "%s tag expected 'hide' as last argument" % name)
        return NoAxisNode(template.Variable(side))

    else:
        raise template.TemplateSyntaxError(
            "axis tag takes one or two arguments")
...
def norm(n, value_range):
    ...
    if minvalue == maxvalue == 0:
        ...
        return int(
            round((n - minvalue) * (float(4095) / (maxvalue - minvalue))))
...
def smart_join(sep, *args):
    return sep.join(
        smart_str(s, errors="ignore") for s in args if s is not None)
